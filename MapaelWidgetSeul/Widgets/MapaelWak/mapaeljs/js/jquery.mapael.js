/**
 *
 * Jquery Mapael - Dynamic maps jQuery plugin (based on raphael2.js)
 * Requires jQuery and raphael2.js
 *
 * Version: 1.1.0
 *
 ...
 */ (function($) {
    ...
    $.fn.mapael = function(options) {
        ...
        return this.each(function() {
            ...
            var $self = $(this),
//                $boolBaseConstruct = !options.map.boolWakanded,
                ...
                paper2 = new Raphael2($container[0], mapConf.width, mapConf.height),
                elemOptions = {},
                resizeTO = 0,
                areas = {},
                plots = {},
            ...
            paper2.setViewBox(0, 0, mapConf.width, mapConf.height, false);
            //			debugger;
            // Draw map areas
            for (id in mapConf.elems) {
                elemOptions = $.fn.mapael.getElemOptions(
                options.map.defaultArea, (options.areas[id] ? options.areas[id] : {}), options.legend.area);
                areas[id] = {
                    "mapElem": paper2.path(mapConf.elems[id]).attr(elemOptions.attrs)
                };
                //nous créons la zone cliquable équivalente dans la base de données
                //                	 debugger;
                NameSpace1.isEmptyAsync({ // Asynchronous call
                    ...
                    'params': {
                        'id': id,
                        'path': mapConf.elems[id],
                        'mapName': mapName
                    }
                });
                //                sources.areas.addEntity();
                //                var myArea = sources.areas.getCurrentElement();
                //                myArea.areaCarte.setValue("bernard");
                //                myArea.areaPathName.setValue(id);
                //                myArea.areaPath.setValue(mapConf.elems[id]);
                //                myArea.areaCarte=mapName;
                //                myArea.areaPathName=(id);
                //                myArea.areaPath=(mapConf.elems[id]);                
                //                myArea.save()
            }

            // Hook that allows to add custom processing on the map
            options.map.beforeInit && options.map.beforeInit($self, paper2, options);

            // Init map areas in a second loop (prevent texts to be hidden by map elements)
            for (id in mapConf.elems) {
                elemOptions = $.fn.mapael.getElemOptions(
                options.map.defaultArea, (options.areas[id] ? options.areas[id] : {}), options.legend.area);
                $.fn.mapael.initElem(paper2, areas[id], elemOptions, $tooltip, id);
            }

            // Draw links
            links = $.fn.mapael.drawLinksCollection(paper2, options, options.links, mapConf.getCoords, $tooltip);

            // Draw plots
            for (id in options.plots) {
                plots[id] = $.fn.mapael.drawPlot(id, options, mapConf, paper2, $tooltip);
            }
            ...
            $self.on("zoom", function(e, zoomOptions) {
                ...
                if (typeof zoomOptions.x == "undefined") zoomOptions.x = paper2._viewBox[0] + paper2._viewBox[2] / 2;

                if (typeof zoomOptions.y == "undefined") zoomOptions.y = (paper2._viewBox[1] + paper2._viewBox[3] / 2);
                ...
                if (options.map.zoom.animDuration > 0) {
                    $.fn.mapael.animateViewBox($container, paper2, panX, panY, mapConf.width / zoomLevel, mapConf.height / zoomLevel, options.map.zoom.animDuration, options.map.zoom.animEasing);
                }
                else {
                    paper2.setViewBox(panX, panY, mapConf.width / zoomLevel, mapConf.height / zoomLevel);
                    clearTimeout($.fn.mapael.zoomTO);
                    $.fn.mapael.zoomTO = setTimeout(function() {
                        $container.trigger("afterZoom", {
                            x1: panX,
                            y1: panY,
                            x2: (panX + (mapConf.width / zoomLevel)),
                            y2: (panY + (mapConf.height / zoomLevel))
                        });
                    }, 150);
                }
            ...
            options.map.zoom.enabled && options.map.zoom.mousewheel && $self.on("mousewheel", function(e) {
                ...
                $self.trigger("zoom", {
                    "fixedCenter": true,
                    "level": $self.data("zoomLevel") + zoomLevel,
                    "x": x,
                    "y": y
                });
            ...
            // Enable zoom
            if (options.map.zoom.enabled) $.fn.mapael.initZoom($container, paper2, mapConf.width, mapConf.height, options.map.zoom);
            ...
            $self.on("update", function(e, updatedOptions, newPlots, deletedPlots, opt) {
                ...
                if (typeof deletedPlots == "object") {
                    for (; i < deletedPlots.length; i++) {
                        if (typeof plots[deletedPlots[i]] != "undefined") {
                            if (animDuration > 0) {
                                (function(plot) {
                                    plot.mapElem.animate({
                                        "opacity": 0
                                    }, animDuration, "linear", function() {
                                        plot.mapElem.remove();
                                    });
                                    if (plot.textElem) {
                                        plot.textElem.animate({
                                            "opacity": 0
                                        }, animDuration, "linear", function() {
                ...
                if (typeof newPlots == "object") {
                    for (id in newPlots) {
                        if (typeof plots[id] == "undefined") {
                            ...
                            plots[id] = $.fn.mapael.drawPlot(id, options, mapConf, paper2, $tooltip);
                            if (animDuration > 0) {
                                plots[id].mapElem.attr({
                                    opacity: 0
                                });
                                plots[id].mapElem.animate({
                                    "opacity": (typeof plots[id].mapElem.originalAttrs.opacity != "undefined") ? plots[id].mapElem.originalAttrs.opacity : 1
                                }, animDuration);

                                if (plots[id].textElem) {
                                    plots[id].textElem.attr({
                                        opacity: 0
                                    });
                                    plots[id].textElem.animate({
                                        "opacity": (typeof plots[id].textElem.originalAttrs.opacity != "undefined") ? plots[id].textElem.originalAttrs.opacity : 1
                                    }, animDuration);
                                }
                            }
                        }
                ...
                if (typeof opt != "undefined" && typeof opt.newLinks == "object") {
                    var newLinks = $.fn.mapael.drawLinksCollection(paper2, options, opt.newLinks, mapConf.getCoords, $tooltip);
                    $.extend(links);
                    if (animDuration > 0) {
                        for (id in newLinks) {
                            newLinks[id].mapElem.attr({
                                opacity: 0
                            });
                            newLinks[id].mapElem.animate({
                                "opacity": (typeof newLinks[id].mapElem.originalAttrs.opacity != "undefined") ? newLinks[id].mapElem.originalAttrs.opacity : 1
                            }, animDuration);

                            if (newLinks[id].textElem) {
                                newLinks[id].textElem.attr({
                                    opacity: 0
                                });
                                newLinks[id].textElem.animate({
                                    "opacity": (typeof newLinks[id].textElem.originalAttrs.opacity != "undefined") ? newLinks[id].textElem.originalAttrs.opacity : 1
                                }, animDuration);
                            }
                        }
                    }
                ...
                for (id in plots) {
                    elemOptions = $.fn.mapael.getElemOptions(
                    options.map.defaultPlot, (options.plots[id] ? options.plots[id] : {}), options.legend.plot);
                ...
                for (id in links) {
                    elemOptions = $.fn.mapael.getElemOptions(
                    options.map.defaultLink, (options.links[id] ? options.links[id] : {}), {});

                    $.fn.mapael.updateElem(elemOptions, links[id], $tooltip, animDuration);
                }

                if (typeof opt != "undefined") opt.afterUpdate && opt.afterUpdate($self, paper2, areas, plots, options);
            });

            // Handle resizing of the map
            if (options.map.width) {
                paper2.setSize(options.map.width, mapConf.height * (options.map.width / mapConf.width));
                ...
                $container.on("resizeEnd", function() {
                    ...
                    if (paper2.width != containerWidth) {
                        paper2.setSize(containerWidth, mapConf.height * (containerWidth / mapConf.width));
                    }
                }).on("resizeEnd", createPlotLegend).trigger("resizeEnd");
            }

            // Hook that allows to add custom processing on the map
            options.map.afterInit && options.map.afterInit($self, paper2, areas, plots, options);

            $(paper2.desc).append(" and Mapael (http://www.vincentbroute.fr/mapael/)");
//            $boolBaseConstruct = false;
        });
    };
     ...
     */
    $.fn.mapael.initElem = function(paper2, elem, options, $tooltip, id) {
        var bbox = {},
            textPosition = {};
        if (typeof options.value != "undefined") elem.value = options.value;
        ...
        if (options.text && typeof options.text.content != "undefined") {
            ...
            elem.textElem = paper2.text(textPosition.x, textPosition.y, options.text.content).attr(options.text.attrs);
            $.fn.mapael.setHoverOptions(elem.textElem, options.text.attrs, options.text.attrsHover);
            options.eventHandlers && $.fn.mapael.setEventHandlers(id, options, elem.mapElem, elem.textElem);
            $.fn.mapael.setHover(paper2, elem.mapElem, elem.textElem);
            $(elem.textElem.node).attr("data-id", id);
        }
        else {
            options.eventHandlers && $.fn.mapael.setEventHandlers(id, options, elem.mapElem);
            $.fn.mapael.setHover(paper2, elem.mapElem);
        }
    ...
    /**
     * Draw all links between plots on the paper2
     */
    $.fn.mapael.drawLinksCollection = function(paper2, options, linksCollection, getCoords, $tooltip) {
        var p1 = {},
            p2 = {},
            elemOptions = {},
            coordsP1 = {},
        ...
        for (var id in linksCollection) {
            ...
            links[id] = $.fn.mapael.drawLink(id, paper2, coordsP1.x, coordsP1.y, coordsP2.x, coordsP2.y, elemOptions, $tooltip);
        }
        return links;
    };

    /**
     * Draw a curved link between two couples of coordinates a(xa,ya) and b(xb, yb) on the paper2
     */
    $.fn.mapael.drawLink = function(id, paper2, xa, ya, xb, yb, elemOptions, $tooltip) {
        var elem = {}
        ...
        //		debugger;
        elem.mapElem = paper2.path("m " + xa + "," + ya + " C " + x + "," + y + " " + xb + "," + yb + " " + xb + "," + yb + "").attr(elemOptions.attrs);
        $.fn.mapael.initElem(paper2, elem, elemOptions, $tooltip, id);
    ...
    $.fn.mapael.updateElem = function(elemOptions, elem, $tooltip, animDuration) {
        ...
        if (typeof elemOptions.value != "undefined") elem.value = elemOptions.value;

        // Update the label
        if (elem.textElem) {
            if (typeof elemOptions.text != "undefined" && typeof elemOptions.text.content != "undefined" && elemOptions.text.content != elem.textElem.attrs.text) elem.textElem.attr({
            ...
            if (animDuration > 0) elem.textElem.animate(elemOptions.text.attrs, animDuration);
            else elem.textElem.attr(elemOptions.text.attrs);
        }

        // Update elements attrs and attrsHover
        $.fn.mapael.setHoverOptions(elem.mapElem, elemOptions.attrs, elemOptions.attrsHover);
        if (animDuration > 0) elem.mapElem.animate(elemOptions.attrs, animDuration);
        else elem.mapElem.attr(elemOptions.attrs);
     ...
     * Draw the plot
     */
    $.fn.mapael.drawPlot = function(id, options, mapConf, paper2, $tooltip) {
        var plot = {},
            coords = {},
            elemOptions = $.fn.mapael.getElemOptions(
            options.map.defaultPlot, (options.plots[id] ? options.plots[id] : {}), options.legend.plot);

        if (typeof elemOptions.x != "undefined" && typeof elemOptions.y != "undefined") coords = {
            x: elemOptions.x,
            y: elemOptions.y
        };
        else coords = mapConf.getCoords(elemOptions.latitude, elemOptions.longitude);

        if (elemOptions.type == "square") {
            plot = {
                "mapElem": paper2.rect(
                coords.x - (elemOptions.size / 2), coords.y - (elemOptions.size / 2), elemOptions.size, elemOptions.size).attr(elemOptions.attrs)
            };
        }
        else if (elemOptions.type == "image") {
            plot = {
                "mapElem": paper2.image(
                elemOptions.url, coords.x - elemOptions.width / 2, coords.y - elemOptions.height / 2, elemOptions.width, elemOptions.height).attr(elemOptions.attrs)
            };
        }
        else if (elemOptions.type == "svg") {
            //			debugger;
            plot = {
                "mapElem": paper2.path(elemOptions.path).attr(elemOptions.attrs)
            };
            ...
            plot = {
                "mapElem": paper2.circle(coords.x, coords.y, elemOptions.size / 2).attr(elemOptions.attrs)
            };
        }

        $.fn.mapael.initElem(paper2, plot, elemOptions, $tooltip, id);
        return plot;
    };
    ...
    $.fn.mapael.setHref = function(elem) {
        ...
        $(elem.node).bind("dblclick", function() {
            debugger;
            if (!$.fn.mapael.panning && elem.href) window.open(elem.href, elem.target);
        });
    };
    ...
    $.fn.mapael.setTooltip = function(elem, $tooltip) {
        var tooltipTO = 0,
            ...
            $switchToInput = function() {
                ...
                //                    if (e.which == 13) {
                //                        debugger;
                //                        $(elem.node).trigger("mouseover");
                //                        elem.tooltip.content = e.target.value;
                //                    }
                //                });
            ...
            $switchToNormal = function() {
                debugger;
                event.stopPropagation();                
                $(elem.node).on('mousemove', $elemMove);
                $(elem.node).on('mouseover', $elemOver);
                $(elem.node).on('mouseout', $elemOut);
                $(elem.node).trigger('mouseout.number1');
                $(elem.node).trigger('mouseout.number2');
                $(elem.node).on('click', $elemClick);
//                $(elem.node).trigger('mouseout', outBehaviour());
                elem.tooltip.content = $tooltip.html();
                ...
                function() {
                    ...
                    if (typeof elem.tooltip != "undefined") {
                        if (typeof elem.tooltip.content != "undefined") {
                            //on va réorganiser en champs <span> les options d'origine
                        var myString = elem.tooltip.content;                            
                            if (typeof elem.tooltip.misAJour=="undefined") {
                                var myRegexp = /style/;
                                ...
                                myStringtoInserted = "<span id=\"1\">" + myresult[0] + "<\/span><br \/>Enfants : <span id=\"2\">146254<\/span><\/span><br \/>Adultes :<span id=\"3\">145258<\/span>";
                                myString = myString.replace(myresult[0], myStringtoInserted);
                                elem.tooltip.content=myString;
                                elem.tooltip.misAJour=true;
                            }
                            //                        elem.tooltip.content=myString;
                            $tooltip.html(myString).css("display", "block");
                        }
            ...
            $elemClick = function(e) {
                $(elem.node).off('mousemove');
                $(elem.node).off('mouseover');
                $(elem.node).off('mouseout');
                $tooltip.on("mouseleave", $switchToNormal);
                $("span").on("click", $switchToInput);
                $("span:eq(0)").trigger("click")
            },
            cssClass = $tooltip.attr('class');

        $(elem.node).on("mouseover", $elemOver).on("mouseout.number2", $elemOut).on("mousemove", $elemMove).on("click", $elemClick);
        ...
        for (var event in elemOptions.eventHandlers) {
            (function(event) {
                $(mapElem.node).on(event, function(e) {
                    !$.fn.mapael.panning && elemOptions.eventHandlers[event](e, id, mapElem, textElem, elemOptions)
                });
     ...
     * @param options
     */
    $.fn.mapael.initZoom = function($container, paper2, mapWidth, mapHeight, options) {
        ...
        // Zoom
        $parentContainer.data("zoomLevel", 0).data({
            "panX": 0,
            "panY": 0
        });
        ...
        $zoomIn.on("click", function() {
            $parentContainer.trigger("zoom", {
                "level": $parentContainer.data("zoomLevel") + 1
            });
        });
        ...
        $container.on("mousedown" + (options.touch ? " touchstart" : ""), function(e) {
            ...
            if (mousedown && currentLevel != 0) {
                var offsetX = (previousX - pageX) / (1 + (currentLevel * options.step)) * (mapWidth / paper2.width),
                    offsetY = (previousY - pageY) / (1 + (currentLevel * options.step)) * (mapHeight / paper2.height),
                    panX = Math.min(Math.max(0, paper2._viewBox[0] + offsetX), (mapWidth - paper2._viewBox[2])),
                    panY = Math.min(Math.max(0, paper2._viewBox[1] + offsetY), (mapHeight - paper2._viewBox[3]));
                ...
                if (Math.abs(offsetX) > 5 || Math.abs(offsetY) > 5) {
                    ...
                    paper2.setViewBox(panX, panY, paper2._viewBox[2], paper2._viewBox[3]);

                    clearTimeout($.fn.mapael.panningTO);
                    $.fn.mapael.panningTO = setTimeout(function() {
                        $container.trigger("afterPanning", {
                            x1: panX,
                            y1: panY,
                            x2: (panX + paper2._viewBox[2]),
                            y2: (panY + paper2._viewBox[3])
                        });
                    }, 150);
    ...
    $.fn.mapael.drawLegend = function(legendOptions, $container, options, legendType, elems, scale, legendIndex) {
        ...
        if (!legendOptions.slices || !legendOptions.display) return;

        $legend = $("." + legendOptions.cssClass, $container).empty();
        paper2 = new Raphael2($legend.get(0));
        height = width = 0;

        // Set the title of the legend
        if (legendOptions.title) {
            title = paper2.text(legendOptions.marginLeftTitle, 0, legendOptions.title).attr(legendOptions.titleAttrs);
            title.attr({
        ...
        for (i = 0, length = legendOptions.slices.length; i < length; ++i) {
            if (typeof legendOptions.slices[i].legendSpecificAttrs == "undefined") legendOptions.slices[i].legendSpecificAttrs = {};

            sliceAttrs[i] = $.extend({}, (legendType == "plot") ? options.map["defaultPlot"].attrs : options.map["defaultArea"].attrs, legendOptions.slices[i].attrs, legendOptions.slices[i].legendSpecificAttrs);

            if (legendType == "area") {
                if (typeof sliceAttrs[i].width == "undefined") sliceAttrs[i].width = 30;
                if (typeof sliceAttrs[i].height == "undefined") sliceAttrs[i].height = 20;
            }
            else if (legendOptions.slices[i].type == "square") {
                if (typeof sliceAttrs[i].width == "undefined") sliceAttrs[i].width = legendOptions.slices[i].size;
                if (typeof sliceAttrs[i].height == "undefined") sliceAttrs[i].height = legendOptions.slices[i].size;
            }
            else if (legendOptions.slices[i].type == "image" || legendOptions.slices[i].type == "svg") {
                if (typeof sliceAttrs[i].width == "undefined") sliceAttrs[i].width = legendOptions.slices[i].width;
                if (typeof sliceAttrs[i].height == "undefined") sliceAttrs[i].height = legendOptions.slices[i].height;
            }
            else {
                if (typeof sliceAttrs[i].r == "undefined") sliceAttrs[i].r = legendOptions.slices[i].size / 2;
            }
        ...
        for (i = 0, length = legendOptions.slices.length; i < length; ++i) {
            if (typeof legendOptions.slices[i].display == "undefined" || legendOptions.slices[i].display == true) {
                if (legendType == "area") {
                    ...
                    elem = paper2.rect(x, y, scale * (sliceAttrs[i].width), scale * (sliceAttrs[i].height));
                }
                else if (legendOptions.slices[i].type == "square") {
                    if (legendOptions.mode == "horizontal") {
                        x = width + legendOptions.marginLeft;
                    ...
                    if (legendOptions.slices[i].type == "image") {
                        ...
                        elem = paper2.path(legendOptions.slices[i].path);
                    ...
                    elem = paper2.circle(x, y, scale * (sliceAttrs[i].r));
                }
                ...
                label = paper2.text(x, y, legendOptions.slices[i].label).attr(legendOptions.labelAttrs);

                // Update the width and height for the paper2
                if (legendOptions.mode == "horizontal") {
                ...
                if (legendOptions.hideElemsOnClick.enabled) {
                    ...
                    $.fn.mapael.setHover(paper2, elem, label);
        ...
        if (Raphael2.type != "SVG" && legendOptions.VMLWidth) width = legendOptions.VMLWidth;
    ...
    $.fn.mapael.handleClickOnLegendElem = function($container, legendOptions, sliceOptions, label, elem, elems, legendIndex) {
        var hideMapElems = function(e, hideOtherElems) {
                ...
                for (var id in elems) {
                    ...
                    if ((typeof sliceOptions.sliceValue != "undefined" && elemValue == sliceOptions.sliceValue) || ((typeof sliceOptions.sliceValue == "undefined") && (typeof sliceOptions.min == "undefined" || elemValue >= sliceOptions.min) && (typeof sliceOptions.max == "undefined" || elemValue < sliceOptions.max))) {
                        (function(id) {
                            if (hidden == 0) {
                                elems[id].mapElem.animate({
                                    "opacity": legendOptions.hideElemsOnClick.opacity
                                }, 300, "linear", function() {
                                    (legendOptions.hideElemsOnClick.opacity == 0) && elems[id].mapElem.hide();
                                });
                                elems[id].textElem && elems[id].textElem.animate({
                                    "opacity": legendOptions.hideElemsOnClick.opacity
                                }, 300, "linear", function() {
                                ...
                                elems[id].mapElem.animate({
                                    "opacity": typeof elems[id].mapElem.originalAttrs.opacity != "undefined" ? elems[id].mapElem.originalAttrs.opacity : 1
                                }, 300);
                                elems[id].textElem && elems[id].textElem.animate({
                                    "opacity": typeof elems[id].textElem.originalAttrs.opacity != "undefined" ? elems[id].textElem.originalAttrs.opacity : 1
                                }, 300);
                            }
                        })(id);
                    }
                ...
                if ((typeof hideOtherElems === "undefined" || hideOtherElems === true) && typeof legendOptions.exclusive !== "undefined" && legendOptions.exclusive === true) {
    ...
    $.fn.mapael.setHoverOptions = function(elem, originalAttrs, attrsHover) {
        ...
        if (Raphael2.type != "SVG") delete attrsHover.transform;
     ...
     * @param paper2 Raphael2 paper2 object
     * @param mapElem the map element
     * @param textElem the optional text element (within the map element)
     */
    $.fn.mapael.setHover = function(paper2, mapElem, textElem) {
        var $mapElem = {},
            $textElem = {},
            hoverTO = 0,
            overBehaviour = function() {
                hoverTO = setTimeout(function() {
                    $.fn.mapael.elemHover(paper2, mapElem, textElem);
                }, 120);
            },
            outBehaviour = function() {
                clearTimeout(hoverTO);
                $.fn.mapael.elemOut(paper2, mapElem, textElem);
            };
        ...
        $mapElem.on("mouseout.number1", outBehaviour);
     ...
     * @param paper2 paper2 Raphael2 paper2 object
     * @param mapElem mapElem the map element
     * @param textElem the optional text element (within the map element)
     */
    $.fn.mapael.elemHover = function(paper2, mapElem, textElem) {
     ...
     * @param textElem the optional text element (within the map element)
     */
    $.fn.mapael.elemOut = function(paper2, mapElem, textElem) {
    ...
    $.fn.mapael.getLegendSlice = function(value, legend) {
        for (var i = 0, length = legend.slices.length; i < length; ++i) {
            if ((typeof legend.slices[i].sliceValue != "undefined" && value == legend.slices[i].sliceValue) || ((typeof legend.slices[i].sliceValue == "undefined") && (typeof legend.slices[i].min == "undefined" || value >= legend.slices[i].min) && (typeof legend.slices[i].max == "undefined" || value < legend.slices[i].max))) {
     ...
     * @param easying_function defined Raphael2 supported easing_formula to use
     * @param callback method when animated action is complete
     */
    $.fn.mapael.animateViewBox = function animateViewBox($container, paper2, x, y, w, h, duration, easingFunction) {
        var cx = paper2._viewBox ? paper2._viewBox[0] : 0,
            dx = x - cx,
            cy = paper2._viewBox ? paper2._viewBox[1] : 0,
            dy = y - cy,
            cw = paper2._viewBox ? paper2._viewBox[2] : paper2.width,
            dw = w - cw,
            ch = paper2._viewBox ? paper2._viewBox[3] : paper2.height,
            dh = h - ch,
            easingFunction = easingFunction || "linear",
            interval = 25,
            steps = duration / interval,
            current_step = 0,
            easingFormula = Raphael2.easing_formulas[easingFunction];

        clearInterval($.fn.mapael.animationIntervalID);

        $.fn.mapael.animationIntervalID = setInterval(function() {
            var ratio = current_step / steps;
            paper2.setViewBox(cx + dx * easingFormula(ratio), cy + dy * easingFormula(ratio), cw + dw * easingFormula(ratio), ch + dh * easingFormula(ratio), false);
